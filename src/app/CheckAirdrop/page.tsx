...
const CheckAirdropPage: React.FC = () => {
  ...
  try {
    setLoading(true);
    const { data } = await axios.post("/api/honey/checkAirdrop", {
    ...
    if (data.eligible) {
      ...
      setSuccessMessage(
        `Eligible. You have ${data.rewards.toLocaleString()} HONEY.`
      );
    } else {
      setError("Address not eligible for the airdrop.");
    }
  } catch {
    setError("Endpoint error. Try again.");
  } finally {
    setLoading(false);
...
export default CheckAirdropPage;
